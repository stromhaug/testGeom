...
//purpose  :
//=======================================================================
EdgeBuilder::EdgeBuilder (VertexList& VList)
{
...
Standard_Boolean EdgeBuilder::HasArea() const
{
  if (left.IsNull())
...
TopAbs_State EdgeBuilder::AreaState() const
{
  TopAbs_State stat = TopAbs_UNKNOWN;
  if (!left.IsNull())
...
TopAbs_State EdgeBuilder::AreaEdgeState() const
{
  TopAbs_State stat = TopAbs_UNKNOWN;
  if (!left.IsNull())
    stat = left->EdgeAfter();
...
Handle(AreaLimit) EdgeBuilder::LeftLimit() const
{
  return left;
}
...
Handle(AreaLimit) EdgeBuilder::RightLimit() const
{
  return right;
}
...
void  EdgeBuilder::Builds(const TopAbs_State ToBuild)
{
  toBuild = ToBuild;
  InitAreas();
  do {
...
Standard_Boolean  EdgeBuilder::MoreEdges() const
{
  return HasArea();
}
...
Standard_Boolean  EdgeBuilder::MoreVertices() const
{
  return (current < 3);
}
...
const HLRAlgo_Intersection& EdgeBuilder::Current() const
{
  if (current == 1)
    return left->Vertex();
  else if (current == 2)
...
Standard_Boolean  EdgeBuilder::IsBoundary() const
{
  if (current == 1)
...
Standard_Boolean  EdgeBuilder::IsInterference() const
{
  if (current == 1)
...
TopAbs_Orientation  EdgeBuilder::Orientation() const
{
  if (current == 1) {
