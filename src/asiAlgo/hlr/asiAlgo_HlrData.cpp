...
#include <asiAlgo_HlrEdgeFaceTool.h>
#include <asiAlgo_HlrFaceData.h>
...
IMPLEMENT_STANDARD_RTTIEXT(Data,Standard_Transient)
...
static void AdjustParameter (EdgeData* E,
...
Data::Data (const Standard_Integer NV,
...
void Data::Write (const Handle(Data)& DS,
			  ...
			  const Standard_Integer df)
{
  ...
  EdgeData* ed = &(myEData         .ChangeValue(de));
  EdgeData* e1 = &(DS->EDataArray().ChangeValue(0 ));
  ed++;
  e1++;

  FaceData* fd = &(myFData         .ChangeValue(df));
  FaceData* f1 = &(DS->FDataArray().ChangeValue(0 ));
  fd++;
  f1++;
...
void Data::Update (const HLRAlgo_Projector& P)
{
  myProj = P;
...
//  for (Standard_Integer edge = 1; edge <= myNbEdges; edge++) {
  ...
  for ( edge = 1; edge <= myNbEdges; edge++) {
    EdgeData& ed = myEData.ChangeValue(edge);
    Curve& EC = ed.ChangeGeometry();
    EC.Projector(&myProj);
...
// Linux warning :  assignment to `int' from `double'. Cast has been added.
    ...
    if (ed.Vertical()) {
      ...
      for (Standard_Integer ebis = 1; ebis <= myNbEdges; ebis++) {
        EdgeData& eb = myEData.ChangeValue(ebis);
        if (vsta == eb.VSta()) {
	  eb.VSta    (vend);
	  eb.OutLVSta(vout);
	  eb.CutAtSta(vcut);
  ...
  for (Standard_Integer face = 1; face <= myNbFaces; face++) {
    ...
    FaceData& fd = myFData.ChangeValue(face);
    Surface& FS = fd.Geometry();
    ...
    for (myFaceItr1.InitEdge(fd);
	 ...
	 myFaceItr1.NextEdge()) {
      myFE = myFaceItr1.Edge();
      EdgeData& EDataFE2 = myEData(myFE);
...
void
Data::InitBoundSort (const HLRAlgo_EdgesBlock::MinMaxIndices& MinMaxTot,
...
//purpose  :
//=======================================================================
void Data::InitEdge (const Standard_Integer FI,
			     BRepTopAdaptor_MapOfShapeTool& MST)
{
  ...
  iFaceType   =   ((Surface*)iFaceGeom)->GetType();
  iFaceTest   = !iFaceSimp;
  mySLProps.SetSurface(iFaceGeom);
  myIntersector.Load(iFaceGeom);


  Surface  *p1 = (Surface*)iFaceGeom;
  const BRepAdaptor_Surface& bras=p1->AccessSurface();
...
Standard_Boolean Data::MoreEdge ()
{
...
void Data::NextEdge (const Standard_Boolean skip)
{
  ...
  if (iFaceTest) {
    ...
    myLETol      = myLEData->Tolerance();
    myLEType     = myLEGeom->GetType();
    if (((EdgeData*)myLEData)->Vertical() ||
	(myLEDouble &&
	 ((EdgeData*)myLEData)->HideCount() == myHideCount-1))
      NextEdge();
    ((EdgeData*)myLEData)->HideCount(myHideCount-1);
    return;
  }
  ...
  if (((EdgeData*)myLEData)->HideCount() > myHideCount-2) {
    NextEdge();
    return;
  }
  if (((EdgeData*)myLEData)->Status().AllHidden()) {
    NextEdge();
    return;
  }
...
Standard_Integer Data::Edge () const
{
...
void Data::InitInterference ()
{
  myLLProps.SetCurve(myLEGeom);
  myFaceItr1.InitEdge(*((FaceData*)iFaceData));
...
//  Standard_Integer maxiWire1,maxiWire2,maxiWire3,maxiWire4;
  ...
  while (!MoreInterference() && myFaceItr1.MoreEdge()) {
    ...
    myFEGeom = &(((EdgeData*)myFEData)->ChangeGeometry());
    myFETol  =   ((EdgeData*)myFEData)->Tolerance();
    myFEType =   ((Curve   *)myFEGeom)->GetType();


    if (myFEOri == TopAbs_FORWARD ||
	myFEOri == TopAbs_REVERSED) {
      // Edge from the boundary
      if (!((EdgeData*)myFEData)->Vertical() && !(myFEDouble && !myFEOutLine)) {
	// not a vertical edge and not a double Edge
	HLRAlgo_EdgesBlock::MinMaxIndices* MinMaxFEdg = &((EdgeData*)myFEData)->MinMax();
	//-- -----------------------------------------------------------------------
	...
	if(((TableauRejection *)myReject)->
	  ...
	  if (((MinMaxFEdg->Max[0] - myLEMinMax->Min[0]) & 0x80008000) == 0 &&
	    ...
	    if (myLE == myFE) { // test if an auto-intersection is not useful
	      if (((EdgeData*)myLEData)->AutoIntersectionDone()) {
		((EdgeData*)myLEData)->
		  AutoIntersectionDone(Standard_True);
		if (((EdgeData*)myLEData)->Simple()) {
		  rej = Standard_True;
		}
	      }
	    }
		...
		if(NoInter) {
		  ...
		  if (myIntersector.IsDone()) {
		    ...
		    myNbPoints = myNbSegments = 0;
#ifdef OCCT_DEBUG
		    std::cout << "Data::NextInterference : ";
		    if (myLE == myFE)
		      std::cout << "Edge " << myLE
...
Standard_Boolean Data::RejectedInterference ()
{
  if (iInterf <= myNbPoints) {
    ...
    if (firstPoint)
      nseg++;
    Standard_Real pf = ((Curve*)myLEGeom)->Parameter3d
      (myIntersector.Segment(nseg).FirstPoint().ParamOnFirst());
    Standard_Real pl = ((Curve*)myLEGeom)->Parameter3d
...
Standard_Boolean Data::AboveInterference ()
{ return myAboveIntf; }
...
void Data::LocalLEGeometry2D (const Standard_Real Param,
				      gp_Dir2d& Tg,
				      gp_Dir2d& Nm,
				      Standard_Real& Cu)
{
  ...
  if (!myLLProps.IsTangentDefined())
    throw Standard_Failure("Data::LocalGeometry2D");
...
void Data::LocalFEGeometry2D (const Standard_Integer FE,
				      const Standard_Real Param,
				      gp_Dir2d& Tg,
				      gp_Dir2d& Nm,
				      Standard_Real& Cu)
{
  const Curve* aCurve = &myEData(FE).ChangeGeometry();
...
void Data::EdgeState (const Standard_Real p1,
                              ...
                              TopAbs_State& staft)
{
  ...
  if (EdgeFaceTool::UVPoint(p2,myFEGeom,iFaceGeom,pu,pv))
  {
    ...
    if (mySLProps.IsNormalDefined())
    {
      ...
      gp_Pnt Pbid;
      gp_Vec TngEdge;
      ((Curve*)myLEGeom)->D1(p1,Pbid,TngEdge);
      ...
      staft = TopAbs_OUT;
#ifdef OCCT_DEBUG
    std::cout << "Data::EdgeState : undefined" << std::endl;
#endif
    }
  }
  else {
...
#ifdef OCCT_DEBUG
      std::cout << "Data::HidingStartLevel : ";
...
TopAbs_State Data::Compare (const Standard_Integer E,
                                    const EdgeData& ED)
{
  Standard_Integer level = 0;
...
Standard_Boolean Data::OrientOutLine (const Standard_Integer I, FaceData& FD)
{
  ...
  for (iw1 = 1; iw1 <= nw; iw1++) {
	...
	if (EdgeFaceTool::UVPoint(p,myFEGeom,iFaceGeom,pu,pv)) {
	  ...
	  V.Transform(TI);
	  if (mySLProps.IsNormalDefined()) {
	    Standard_Real curv = EdgeFaceTool::CurvatureValue
	      (iFaceGeom,pu,pv,V);
	    gp_Vec Nm = mySLProps.Normal();
	    if (curv == 0) {
#ifdef OCCT_DEBUG
	      std::cout << "Data::OrientOutLine " << I;
	      std::cout << " Edge " << myFE << " : ";
...
void Data::OrientOthEdge (const Standard_Integer I,
				  FaceData& FD)
{
  Standard_Real p,pu,pv,r;
  const Handle(HLRAlgo_WiresBlock)& wb = FD.Wires();
...
TopAbs_State
Data::Classify (const Standard_Integer E,
			const EdgeData& ED,
...
//  Standard_Boolean rej = Standard_False;
  ...
  if (myIntersector.IsDone()) {
	...
	if (!myLEOutLine && !myLEInternal) TolZ = myBigSize * 0.001;
      ...
      if (((Surface*)iFaceGeom)->IsUPeriodic()) {
	PeriodU = ((Surface*)iFaceGeom)->UPeriod();
	UMin = ((Surface*)iFaceGeom)->FirstUParameter();
	UMax = ((Surface*)iFaceGeom)->LastUParameter();
      }
      else
	PeriodU = 0.;
      if (((Surface*)iFaceGeom)->IsVPeriodic()) {
	PeriodV = ((Surface*)iFaceGeom)->VPeriod();
	VMin = ((Surface*)iFaceGeom)->FirstVParameter();
	VMax = ((Surface*)iFaceGeom)->LastVParameter();
      }
      else
	PeriodV = 0;
      gp_Pnt PInter;
...
TopAbs_State Data::SimplClassify (const Standard_Integer /*E*/,
					  const EdgeData& ED,
...
Standard_Boolean
Data::RejectedPoint (const IntRes2d_IntersectionPoint& PInter,
			     ...
			     const Standard_Integer NumSeg)
{
  ...
  p1 = ((Curve*)myLEGeom)->Parameter3d(PInter.ParamOnFirst ());
  p2 = ((Curve*)myFEGeom)->Parameter3d(PInter.ParamOnSecond());
  dz = ((Curve*)myLEGeom)->Z(p1)-((Curve*)myFEGeom)->Z(p2);
  ...
  if (iFaceTest) {
    if (myLE == myFE) {
      if (st == TopAbs_IN)
	((EdgeData*)myLEData)->Simple(Standard_False);
    }
    else {
      if (mySameVertex) {
	if ((st == TopAbs_ON)                           ||
  ...
  if (st != TopAbs_OUT) {
	...
	Ind = ((EdgeData*)myFEData)->VSta();
	Or2 = TopAbs_FORWARD ;
	AdjustParameter((EdgeData*)myFEData,Standard_True ,p2,t2);
	if (((EdgeData*)myFEData)->VerAtSta()) {
	  douteux = Standard_True;
	  ((Curve*)myFEGeom)->D2(psav,Ptsav,Tgsav,Nmsav);
	  if (Tgsav.SquareMagnitude() <= DERIVEE_PREMIERE_NULLE)
	    Tgsav = Nmsav;
	}
      }
      else {
	Ind = ((EdgeData*)myFEData)->VEnd();
	Or2 = TopAbs_REVERSED;
	AdjustParameter((EdgeData*)myFEData,Standard_False,p2,t2);
	if (((EdgeData*)myFEData)->VerAtEnd()) {
	  douteux = Standard_True;
	  ((Curve*)myFEGeom)->D2(psav,Ptsav,Tgsav,Nmsav);
	  if (Tgsav.SquareMagnitude() <= DERIVEE_PREMIERE_NULLE)
	    Tgsav = Nmsav;
      ...
      gp_Vec2d TgFE;
      ((Curve*)myFEGeom)->D1(p2,Ptsav,TgFE);
      if (douteux) {
	...
	AdjustParameter((EdgeData*)myLEData,Standard_True ,p1,t1);
	if (((EdgeData*)myLEData)->VerAtSta()) {
	  douteux = Standard_True;
	  ((Curve*)myLEGeom)->D2(psav,Ptsav,Tgsav,Nmsav);
	  if (Tgsav.SquareMagnitude() <= DERIVEE_PREMIERE_NULLE)
	    Tgsav=Nmsav;
	}
      }
      else {
	AdjustParameter((EdgeData*)myLEData,Standard_False,p1,t1);
	if (((EdgeData*)myLEData)->VerAtEnd()) {
	  douteux = Standard_True;
	  ((Curve*)myLEGeom)->D2(psav,Ptsav,Tgsav,Nmsav);
	  if (Tgsav.SquareMagnitude() <= DERIVEE_PREMIERE_NULLE)
	    Tgsav=Nmsav;
      ...
      if (douteux) {
	gp_Vec2d TgLE;
	((Curve*)myLEGeom)->D1(p1,Ptsav,TgLE);
...
Standard_Boolean
Data::SameVertex (const Standard_Boolean h1,
			  const Standard_Boolean h2)
{
  Standard_Integer v1,v2;
  if (h1) v1 = ((EdgeData*)myLEData)->VSta();
  else    v1 = ((EdgeData*)myLEData)->VEnd();
  if (h2) v2 = ((EdgeData*)myFEData)->VSta();
  else    v2 = ((EdgeData*)myFEData)->VEnd();
  ...
  if (SameV) {
    ...
    if (( h1 && ((EdgeData*)myLEData)->OutLVSta()) ||
	(!h1 && ((EdgeData*)myLEData)->OutLVEnd())) {
    ...
    if (otherCase) {
      if (( h1 && ((EdgeData*)myLEData)->CutAtSta()) ||
	  (!h1 && ((EdgeData*)myLEData)->CutAtEnd())) {
