...
void  Intersector::Perform (const Standard_Address A1,
				    ...
				    const Standard_Real db1)
{
  ...
  Curve* myC1 = ((EdgeData*) A1)->AccessCurve();
  ...
  ((EdgeData*) A1)->Status().Bounds(a,ta,b,tb);
  d = b - a;
  if (da1 != 0) a = a + d * da1;
  if (db1 != 0) b = b - d * db1;
  myC1->D0(a,pa);
  ...
  //tol = (Standard_Real)(((EdgeData*) A1)->Tolerance());
...
void  Intersector::Perform (const Standard_Integer /*nA*/,
...
//  }
  ...
  Curve* myC1 = ((EdgeData*) A1)->AccessCurve();
  Curve* myC2 = ((EdgeData*) A2)->AccessCurve();
  ...
  //modified by jgv, 18.04.2016 for OCC27341
  //tol1 = (Standard_Real)(((EdgeData*) A1)->Tolerance());
  //tol2 = (Standard_Real)(((EdgeData*) A2)->Tolerance());
  ...
  do {
    ...
    ((EdgeData*) A1)->Status().Bounds(a1,ta,b1,tb); //--   -> Parametres 3d
...
//    }
    ...
    ((EdgeData*) A2)->Status().Bounds(a2,ta,b2,tb);
    mtol = tol;
    if(mtol<ta) mtol=ta;
    if(mtol<tb) mtol=tb;
...
void  Intersector::SimulateOnePoint(const Standard_Address A1,
...
void  Intersector::Load (Standard_Address& A)
{
  mySurface = A;
...
void  Intersector::Perform (const gp_Lin& L,
				    const Standard_Real P)
{
  myTypePerform = 2;
  GeomAbs_SurfaceType typ = SurfaceTool::GetType(mySurface);
  switch (typ) {
  case GeomAbs_Plane :
  ...
  case GeomAbs_Torus :
	...
	u1   = SurfaceTool::FirstUParameter(mySurface);
	v1   = SurfaceTool::FirstVParameter(mySurface);
	u2   = SurfaceTool::LastUParameter(mySurface);
	v2   = SurfaceTool::LastVParameter(mySurface);
	nbsu = SurfaceTool::NbSamplesU(mySurface,u1,u2);
	nbsv = SurfaceTool::NbSamplesV(mySurface,v1,v2);
	myPolyhedron =
    new PolyhedronOfInterCSurf(mySurface,nbsu,nbsv,u1,v1,u2,v2);
      }
      ...
      if (pmin > P) pmin = P - d;
      if (pmax > P) pmax = P;
      PolygonOfInterCSurf Polygon(L,pmin,pmax,3);
...
Standard_Boolean Intersector::IsDone () const
{
  if(myTypePerform == 1)
...
Standard_Integer Intersector::NbPoints() const
{
...
Intersector::Point (const Standard_Integer N) const
{
  if(myTypePerform==0)
...
const IntCurveSurface_IntersectionPoint &
 Intersector::CSPoint (const Standard_Integer N) const
{
...
Standard_Integer Intersector::NbSegments () const
{
  if(myTypePerform == 1)
...
Intersector::Segment (const Standard_Integer N) const
{
...
Intersector::CSSegment (const Standard_Integer N) const
{
...
void  Intersector::Perform (const Standard_Integer nA,
				    ...
				    const Standard_Boolean EnBout)
{
  Standard_Address myC1 = ((EdgeData*) A1)->Curve();
  Standard_Address myC2 = ((EdgeData*) A2)->Curve();
  ...
  ((EdgeData*) A1)->Status().Bounds(a,ta,b,tb); //--   -> Parametres 3d
  Standard_Real mtol = tol;
  if(mtol<ta) mtol=ta;
  if(mtol<tb) mtol=tb;
  d = b - a;
  ...
  a = ((Curve*)myC1)->Parameter2d(a);
  b = ((Curve*)myC1)->Parameter2d(b);
  ...
  ((EdgeData*) A2)->Status().Bounds(a,ta,b,tb);
  mtol = tol;
  if(mtol<ta) mtol=ta;
  if(mtol<tb) mtol=tb;
  ...
  a = ((Curve*)myC2)->Parameter2d(a);
  b = ((Curve*)myC2)->Parameter2d(b);
