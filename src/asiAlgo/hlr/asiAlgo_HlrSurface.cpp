...
void Surface::SetSurface (const TopoDS_Face& F)
{
  //mySurf.Initialize(F,Standard_False);
  mySurf.Initialize(F,Standard_True);
  GeomAbs_SurfaceType typ = hlr::BSurfaceTool::GetType(mySurf);
  switch (typ) {
  ...
  case GeomAbs_BezierSurface :
    if (hlr::BSurfaceTool::UDegree(mySurf) == 1 &&
            hlr::BSurfaceTool::VDegree(mySurf) == 1) {
...
Standard_Boolean
Surface::SideRowsOfPoles (const Standard_Real tol,
...
Standard_Boolean
Surface::IsSide (const Standard_Real tolF,
			 const Standard_Real toler) const
{
  ...
  if (myType == GeomAbs_Plane) {
    ...
    gp_Cylinder Cyl = hlr::BSurfaceTool::Cylinder(mySurf);
    gp_Ax1 A = Cyl.Axis();
    D  = A.Direction();
    D .Transform(myProj->Transformation());
    ...
    if (!myProj->Perspective()) return Standard_False;
    gp_Cone Con = hlr::BSurfaceTool::Cone(mySurf);
    Pt = Con.Apex();
    Pt.Transform(myProj->Transformation());
    ...
    if (myProj->Perspective()) return Standard_False;
    Standard_Integer nu = hlr::BSurfaceTool::NbUPoles(mySurf);
    Standard_Integer nv = hlr::BSurfaceTool::NbVPoles(mySurf);
    TColgp_Array2OfPnt Pnt(1,nu,1,nv);
    hlr::BSurfaceTool::Bezier(mySurf)->Poles(Pnt);
    ...
    hlr::BSurfaceTool::BSpline(mySurf)->Poles(Pnt);
    hlr::BSurfaceTool::BSpline(mySurf)->Weights(W);
    return SideRowsOfPoles (tolF,nu,nv,Pnt);
  }
...
Standard_Boolean
Surface::IsAbove (const Standard_Boolean back,
			  const Curve* A,
...
gp_Pnt Surface::Value (const Standard_Real U,
...
gp_Pln  Surface::Plane () const
{
  ...
  case GeomAbs_BezierSurface :
    ...
    default :
    return hlr::BSurfaceTool::Plane(mySurf);
  }
}
