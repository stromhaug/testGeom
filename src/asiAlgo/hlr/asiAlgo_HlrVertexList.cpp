...
//=======================================================================
VertexList::
VertexList(const EdgeInterferenceTool& T,
...
Standard_Boolean  VertexList::IsPeriodic()const
{
...
Standard_Boolean  VertexList::More()const
{
...
const HLRAlgo_Intersection &  VertexList::Current() const
{
  if (fromEdge)
    ...
    throw Standard_NoSuchObject("VertexList::Current");
}
...
Standard_Boolean  VertexList::IsBoundary() const
{
  return fromEdge;
}
...
Standard_Boolean  VertexList::IsInterference() const
{
  return fromInterf;
}
...
TopAbs_Orientation  VertexList::Orientation() const
{
  if (fromEdge)
    return myTool.CurrentOrientation();
  else
    throw Standard_DomainError("VertexList::Orientation");
}
...
TopAbs_Orientation  VertexList::Transition() const
{
  if (fromInterf)
    return myIterator.Value().Transition();
  else
    throw Standard_DomainError("VertexList::Transition");
}
...
TopAbs_Orientation  VertexList::BoundaryTransition() const
{
  if (fromInterf)
    return myIterator.Value().BoundaryTransition();
  else
    throw Standard_DomainError("VertexList::BoundaryTransition");
}
